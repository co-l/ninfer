...
std::optional<AdmissionCandidate> ProgramImpl::inspect_lane(
    ...
    if (shared_source != nullptr) {
        ...
        if (!base.allow_prefix_reuse || !prompt.identity.reusable) { return std::nullopt; }
        const auto private_prefix_ok = [&](std::size_t count) -> bool {
            const std::size_t prompt_tokens = prompt.token_ids.size();
            if (count > prompt_tokens || count > source->ledger.size()) { return false; }
            const bool token_eq =
        ...
        if (selected.kind == runtime::CheckpointKind::SessionEndpoint) {
            ...
            if (!private_prefix_ok(selected.frontier)) { return std::nullopt; }
